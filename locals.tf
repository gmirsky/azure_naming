locals {
  ...
  base_name = "${local.prefix}${var.resource_type}${var.dash}${var.workload}${var.dash}${var.environment}${var.dash}${local.region_short}${local.suffix}"
  # Generate name with optional instance number: [prefix-]resourcetype-workload-environment-region-###[-suffix]
  name_with_instance = var.instance_number > 0 ? "${local.prefix}${var.resource_type}${var.dash}${var.workload}${var.dash}${var.environment}${var.dash}${local.region_short}${var.dash}${format("%03d", var.instance_number)}${local.suffix}" : local.base_name
